...
$$ \ell(\beta) = \sum_{i = 1}^n (\hat{y}_i - y_i)^2 + \lambda \sum_{j = 1}^p |\beta| $$
...
$$ \ell(\beta) = SSE + \lambda \left(\sum_{j = 1}^p \beta^2 + \sum_{j = 1}^p |\beta| \right)$$
...
$$ \ell(\beta) = SSE + \lambda \left(\alpha \sum_{j = 1}^p \beta^2 + (1-\alpha) \sum_{j = 1}^p |\beta| \right)$$
...
:::{.callout-warning}
The `sklearn` function `Ridge()` uses the argument name `alpha` for $\lambda$, and the argument name `l1_ratio` for $\alpha$.
