...
The latter (`.head()`) is usually preferred in case the dataset is large.c
...
We can do this with `.sort_values` in either *ascending* or *descending* order. We'll also make use of the `inplace` option so that we're changing the dataset itself, instead of having to create a new variable or reassign the sorted result to our existing `DataFrame` object.
